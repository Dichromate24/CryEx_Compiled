...
# This script calculates the PSI values for ALL combinations of 5SS and 3SS found in an expressed exon
# At the end, each well defined exon with its PSI quantification will be categorized into three types
...
# Using .gtf of the exons, we get the chromosome, start and stop positions
cat ../../../../all_chr_transcripts/CHROME_exons.gtf|awk '{print $1"-"$4"-"$5"-"$12"/""\t"$7}'|tr -d '"'|tr -d ';' > strandedness 
...
join CryEx_coord_len.bed.sort strandedness.sort > CryEx_coord_len.bed.strand
    # this perl command formats it nicely
perl -p -i -e 's/ /\t/g' CryEx_coord_len.bed.strand

# MIDDLE.coord, keep those with BOTH 5SS and 3SS
cat CryEx_coord_len.bed.strand |awk '($3!=0 && $4!=0)'> middle.coord

# FIRST.coord keep those with 5SS and NO 3SS
...
# LAST.coord keep those with 3SS and NO 5SS
...
# NOTSPLICED.coord these have NO 5SS and 3SS
...
### Getting first.psi, the PSI of all first exons ##

# Get the chromosome, start and end, and strandedness for each first exon
...
do
    ...
    then 
        ...
        cat ../sub/${path}final_junc5SS_HTE_count.txt > SS5_file
        ...
        do 
            ...
            do
                ...
                #samtools view -b /mnt/gtklab01/darren/CryEx_toy/HTE1_chr21_sorted.bam "chr21:$SS3-$SS5"> GENE
                samtools view -b ../../../../all_sample_STAR/GSM_SAM_Aligned.sortedByCoord.out.bam "CHROME:$SS3-$SS5"> GENE
    ...
    then
        ...
        cat ../sub/${path}final_junc3SS_HTE_count.txt > SS3_file
...
### Getting last.psi, the PSI of all last exons ###
...
### Getting middle.psi, the PSI of all middle exons ###
...
do
    ...
    then 
        ...
        min_SS3=`echo $(cat ../sub/${path}final_junc3SS_HTE_count.txt|sort -k1,1n|head -1)`
        min_SS3_coord=`echo $min_SS3|awk '{print $1}'`
        min_SS5=`echo $(cat ../sub/${path}final_junc5SS_HTE_count.txt|awk '($1< '"$min_SS3_coord"')')`
        ...
        max_SS5=`echo $(cat ../sub/${path}final_junc5SS_HTE_count.txt|sort -k1,1n|tail -1)`
        max_SS5_coord=`echo $max_SS5|awk '{print $1}'`
        max_SS3=`echo $(cat ../sub/${path}final_junc3SS_HTE_count.txt|awk '($1> '"$max_SS5_coord"')')`
        if [[ ! -z "$max_SS3" ]]
        then
        ...
        do
            ...
            do
                ...
                UP_coord=`echo $ll|awk '{print $2}'`
                UP_count=`echo $ll|awk '{print $4}'`
                cat ../sub/${path}final_junc5SS_HTE_count.txt|awk '($1> '"$SS3"')' > SS5_file
                sud=`echo $(cat ../sub/${path}final_junc5SS_HTE_count.txt|awk '($1> '"$SS3"')')`
                if [[ ! -z "$sud" ]]
                then
...
cat middle_first_plus.PSI middle_first_minus.PSI|awk '!seen[$0]++' > middle_first.PSI
...
mv middle.PSI ../PSI_results
mv middle_minus.PSI ../PSI_results
mv middle_plus.PSI ../PSI_results
mv first.PSI ../PSI_results
mv last.PSI ../PSI_results
